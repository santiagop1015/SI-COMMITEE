...
*/
       	                          ...
       	                          $pdf->SetFont('Arial','B',12);
      	                        //  $pdf->Cell(20, 6, ' ', 0, 0, 'C'); 
       	                          $pdf->Cell(160, 6, 'RESULTADO DE EVALUACION ANTEPROYECTO', 0, 1, 'L'); 
      	                          $pdf->Cell(200, 6, '', 0, 1, 'C');
                                  $pdf->SetFont('Arial','',12);
      	                          $pdf->Cell(20, 6, ' ', 0, 0, 'C'); 
...
if($contador==1){
  $pdf->SetFont('Arial','',9);
  $pdf->Cell(75, 6, 'Jurado '.$contador.' : '. $jurado2.', Fecha: '.$row['fecha_eval'], 1, 1, 'C',1); 
}else {
  $pdf->SetFont('Arial','',9);
  $pdf->Cell(75, 6, 'Jurado '.$contador.' : '. $jurado.', Fecha: '.$row['fecha_eval'], 1, 1, 'C',1); 
}
...
$pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Formulacion del Problema:   '.utf8_decode($row['forprob']), 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Justificacion: : '.utf8_decode($row['justificacion']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C');  
      	                         $pdf->MultiCell(160, 6, 'Objetivos:  '.utf8_decode($row['objetivos']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Marco Referencial : '.utf8_decode($row['marcoref']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Metodologia: '.utf8_decode($row['metodologia']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Cronograma:  '.utf8_decode($row['crono']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Presupuesto:  '.utf8_decode($row['presupuesto']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Bibliografia:  '.utf8_decode($row['biblio']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Cibergrafia:  '.utf8_decode($row['ciber']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Claridad en redaccion:  '.utf8_decode($row['claridad']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
      	                         $pdf->MultiCell(160, 6, 'Evidencia:  '.utf8_decode($row['evidencia']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); $pdf->SetFillColor(232,232,232);
      	                         $pdf->MultiCell(160, 6, 'Concepto:  '.utf8_decode($row['concepto']) , 1, 1, 'L');
      	                         $pdf->Cell(26, 6, ' ', 0, 0, 'C'); $pdf->SetFillColor(255,255,255);
                                 $pdf->MultiCell(160, 6, 'Observaciones:  '.utf8_decode($row['observaciones']) , 1, 1, 'L');
                                 $pdf->Cell(26, 6, ' ', 0, 0, 'C'); 
...
$pdf->SetFont('Arial','',8);
$pdf->Cell(20, 6, ' ', 0, 0, 'C'); 
$pdf->MultiCell(160, 6, 'IMPORTANTE: La presente evaluacion, no sirve para realizar tramites, ante la universidad, solo es de informacion para correcciones de su Anteproyecto. ', 0, 1, 'L',0); 
